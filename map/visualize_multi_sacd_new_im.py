...
from tianshou.policy import SACDMultiPolicy, SACDMultiCCPolicy, SACDMultiWMPolicy, SACDMultiCCWMPolicy, SACDMultiWMPolicyNewIM
from tianshou.data import Collector

from models.world_model_new_im import WorldModelNewIM
...
def visualize_multi_sacd(args=get_args()):
    ...
    world_models = [WorldModelNewIM(num_agents, params.layer_num, params.state_shape, params.wm_hidden_size, device=params.device, wm_noise_level=params.wm_noise_level).to(params.device)
                    for i in range(num_agents)]
    # World Model
    ...
    if params.centralized:
        ...
        if params.intr_rew == 'no':
            policy = SACDMultiPolicy(
                ...
                ignore_done=params.ignore_done,
                estimation_step=params.n_step)
        elif 'new_im' in params.intr_rew:
            policy = SACDMultiWMPolicyNewIM(
                actors, None,
                critic1s, None,
                critic2s, None,
                world_models, None,
