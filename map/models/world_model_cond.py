...
class CondWorldModel(nn.Module):
    def __init__(self, num_agent, layer_num, state_shape, conditional_dim, hidden_units=128, device='cpu', wm_noise_level=0.0):
        super().__init__()
        self.device = device
        # plus one for the action
        self.model = [
            nn.Linear(np.prod(state_shape) + 1 + conditional_dim, hidden_units),
            nn.ReLU()]
