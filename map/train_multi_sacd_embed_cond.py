...
from models.world_model_cond import CondWorldModel
from models.behavior_encoder import MeanEncoder, CovariateEncoder
...
def get_args():
    ...
    parser.add_argument('--conditional_dim', type=int, default=1) # if 1 then uses the agent id number
...
if __name__ == '__main__':
    ...
    # World Model
    world_models = [CondWorldModel(num_agents, args.layer_num, args.state_shape, args.conditional_dim, action_space_n[i], device=args.device, wm_noise_level=args.wm_noise_level).to(args.device)
                    for i in range(num_agents)]
    #wm_optims = [Adam(world_model.parameters(), lr=args.actor_lr)
    #             for world_model in world_models]
    
    # Behavorial Encoders
    behav_mean_encoders = [MeanEncoder(num_agents, args.layer_num, args.state_shape, action_space_n[i], args.history_len, device=args.device).to(args.device)
                      for i in range(num_agents)]
    behav_cov_encoders = [CovariateEncoder(num_agents, args.layer_num, args.state_shape, action_space_n[i], args.history_len, device=args.device).to(args.device)
                      for i in range(num_agents)]
    
    #behav_mean_encoders_optims = [Adam(behav_mean_encoder.parameters(), lr=args.actor_lr)
    #                  for behav_mean_encoder in behav_mean_encoders]
    #behav_cov_encoders_optims = [Adam(behav_cov_encoder.parameters(), lr=args.actor_lr)
    #                  for behav_cov_encoder in behav_cov_encoders]

    joint_optims = [Adam(list(world_model.parameters()) + list(behav_mean_encoder.parameters()) + list(behav_cov_encoder.parameters()), lr=args.actor_lr) 
                       for world_model, behav_mean_encoder, behav_cov_encoder in zip(world_models, behav_mean_encoders, behav_cov_encoders)]
    ...
    if args.centralized:
        ...
        if args.intr_rew == 'no':
            ...
            policy = SACDMultiEmbedCondWMPolicy(
                actors, actor_optims,
                critic1s, critic1_optims,
                critic2s, critic2_optims,
                world_models,
                ...
                behav_cov_encoders,
                joint_optims,
                dist, args.tau, args.gamma, args.alpha, args.history_len,
                reward_normalization=args.rew_norm,
                ignore_done=args.ignore_done,
                estimation_step=args.n_step,
                num_adv=args.num_adversaries,
    ...
    # %%
    # This part runs the trainer (with tqdm)
    # trainer
