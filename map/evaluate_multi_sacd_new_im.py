...
def evaluate_multi_sacd(args=get_args()):
    ...
    #                 for i in range(num_agents)]
    world_models = [WorldModelNewIM(num_agents, params.layer_num, params.state_shape, params.wm_hidden_size, device=params.device, wm_noise_level=params.wm_noise_level).to(args.device)
                    for i in range(num_agents)]
