...
class SACDMultiEmbedCondWMPolicy(BasePolicy):
    ...
    def __init__(self,
                 ...
                 behav_mean_encoders: List[torch.nn.Module],
                 behav_cov_encoders: List[torch.nn.Module],
                 joint_optims: List[torch.optim.Optimizer],
        ...
        self.behav_mean_encoders = behav_mean_encoders
        self.behav_cov_encoders = behav_cov_encoders
        self.joint_optims = joint_optims
        self._alpha = alpha
    ...
    # TODO: nothing, but understand the output for n agents so that you can concatenate the agent id/behavior 
    def imagine_agent_j_obs(self, obs_i, i, j_list):
        #print('obs_i.shape', obs_i.shape, 'obs_i', obs_i)
    ...
    # TODO: modify the concatenation in line 275 to include the agent ids (if I run as is should get a dimension error)
    def calculate_intrinsic_reward(self, batch: Batch) -> Batch:
        #print('Calculating Intrinsic Reward!')
        ...
        if not self.partial_obs or self.intr_rew_options in ['curio_self', 'elign_self']: 
            ...
            for i in range(self.num_adv, num_agents): # apply intr rew only on good agents
                ...
                #print('before world_model(inputs)')
                pred_next_obs_n = world_model(inputs)
                #print('after world_model(inputs)')
        ...
        #print('intr_rew.shape:', intr_rew.shape) # batch_size, num_agents
    ...
    # TODO: modify, each time agent i's dynamics worldmodel is trained, 
    # train it instead by conditioning on it the agent for who its prediction
    # E.g. if its prediction its own observation, then condition on its own id
    # then loop through all other agents, and gather their assumed observations
    # and trained agent i's world model to prediction those observations, conditioned
    # on the other agent's ids, one at at time.
    def learn(self, batch: Batch, **kwargs) -> Dict[str, float]:
        ...
        for i in range(len(self.actors)):
            ...
            ########### Start Train conditional world model and encoders ###############################

            # compute wm loss for agent i
            ...
            if i not in self.train_history.keys():
                self.train_history[i] = np.expand_dims(inputs, axis=1) # put a time dimension for history
            else:
                print(self.train_history[i].shape)
                self.train_history[i] = np.concatenate((np.expand_dims(inputs,axis=1), self.train_history[i]), axis=1)[:,:self.history_len,:] 
            print(self.train_history[i].shape)

            #self.history = np.concatenate((inputs, self.history), axis=1)[:self.history_len] # TODO: make sure this works

            if self.history[i].shape[0] < 4:
                encoder_inputs = np.zeros(shape = (self.train_history[i].shape[0], self.history_len, self.train_history[i].shape[2]))
                encoder_inputs[:, :self.train_history[i].shape[0], :] = self.train_history[i]

                # standard mean and cov
                prior_behav_mean = torch.rand((B, D), requires_grad=True)
                prior_behav_cov = torch.rand((B, D), requires_grad=True)
                prior_dist = torch.distributions.Normal(prior_behav_mean , prior_behav_cov)
                self.prior_dist[i] = prior_dist
            else:
                encoder_inputs = self.train_history[i]


            # compute encoder loss for agent i
            behav_mean_encoder = self.behav_mean_encoders[i]
            behav_cov_encoder = self.behav_cov_encoders[i]
            encoder_inputs = self.train_history[i] # [batch_size, history_len, obs_dim+act_dim]


            behav_mean_encoding = behav_mean_encoder(encoder_inputs)
            behav_cov_encoding = behav_cov_encoder(encoder_inputs)
            behavior_dist = torch.distributions.Normal(behav_mean_encoding, behav_cov_encoding)


            kl_div_loss = torch.distributions.kl_divergence(behavior_dist, self.prior_dist[i]).mean()
            ...
            world_model = self.world_models[i]
            pred_next_obs_n = world_model(inputs)
            pred_losses = torch.square(torch.norm(true_next_obs_n - pred_next_obs_n, p=2, dim=1)) #(bs*j, )
            # zero out the losses for invisible agents
            obs_mask = to_torch(~np.all(obs_n == 0, axis=1), device=self.world_models[i].device, dtype=torch.float)
            pred_losses *= obs_mask
            wm_loss = torch.mean(pred_losses)

            # Joint update to world model and encoder
            joint_wm_and_encoder_loss = wm_loss + kl_div_loss
            self.joint_optims[i].zero_grad()
            joint_wm_and_encoder_loss.backward()
            self.joint_optims[i].step()
            self.prior_dist[i] = behavior_dist.copy()
            ########### End Train conditional world model ###############################
